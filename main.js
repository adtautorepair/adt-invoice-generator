...
ipcMain.handle('ai:improve', async (e, o) => {
  try {
    ...
    const prompt = (o.kind === 'draft')
      ? 'You are a professional service writer at a Canadian auto repair shop. Based only on the list of parts and labour below, write a "work performed" summary for a customer invoice. Aim for a moderate paragraph of about 2 to 4 flowing sentences in warm, professional Canadian English, past tense — keep it concise, not a single terse line and not a bullet list. Describe the services naturally, and you may add brief, generally-true context about the purpose of a standard service (for example, that a tire rotation promotes even tread wear). You may add a short neutral closing such as noting that the work was completed, but do NOT assume or state that the customer will pick up the vehicle or was waiting. Do NOT invent any specific parts, quantities, prices, measurements, findings, problems, or future recommendations that are not in the list. Return only the summary, with no preamble, labels, headings, or quotation marks.\n\nExample:\nItems:\n- Oil change (synthetic) (labour)\n- Tire rotation (labour)\n- Cabin air filter (part)\nOutput: We performed a full synthetic oil change to help keep the engine running smoothly, and rotated the tires to promote even tread wear. The cabin air filter was also replaced to help maintain clean airflow inside the vehicle. All work was completed successfully.\n\nNow write the summary for these items:\n' + o.text
      : (o.kind === 'line')
