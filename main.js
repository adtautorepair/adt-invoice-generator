...
ipcMain.handle('ai:improve', async (e, o) => {
  try {
    ...
    const prompt = (o.kind === 'draft')
      ? 'You are a service writer at a Canadian auto repair shop. Based only on the list of parts and labour below, write a brief, professional "work performed" summary for a customer invoice — one short paragraph in plain, professional Canadian English, past tense. Describe only the items listed; do not invent parts, prices, quantities, or recommendations. Return only the summary, with no preamble, labels, or quotation marks.\n\nExample:\nItems:\n- Oil change (labour)\n- Tire rotation (labour)\n- Cabin air filter (part)\nOutput: Performed a full oil change, rotated the tires, and replaced the cabin air filter.\n\nNow write the summary for these items:\n' + o.text
      : (o.kind === 'line')
