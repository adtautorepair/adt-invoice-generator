...
ipcMain.handle('ai:improve', async (e, o) => {
  try {
    ...
    const prompt = (o.kind === 'draft')
      ? 'You are a professional service writer at a Canadian auto repair shop. Based only on the list of parts and labour below, write a "work performed" summary for a customer invoice. Aim for a moderate paragraph of about 3 to 5 flowing sentences in warm, professional Canadian English, past tense — not a single terse line and not a bullet list. Describe the services naturally, and you may add brief, generally-true context about the purpose of a standard service (for example, that a tire rotation promotes even tread wear) and a short professional closing line (for example, that the work was completed and the vehicle is ready). However, do NOT invent any specific parts, quantities, prices, measurements, findings, problems, or future recommendations that are not in the list. Return only the summary, with no preamble, labels, headings, or quotation marks.\n\nExample:\nItems:\n- Oil change (synthetic) (labour)\n- Tire rotation (labour)\n- Cabin air filter (part)\nOutput: We carried out routine maintenance on the vehicle to help keep it running smoothly and reliably. A full synthetic oil change was performed, and the tires were rotated to promote even tread wear and extend their life. The cabin air filter was also replaced to help maintain clean airflow inside the vehicle. All work has been completed, and the vehicle is ready for pickup.\n\nNow write the summary for these items:\n' + o.text
      : (o.kind === 'line')
